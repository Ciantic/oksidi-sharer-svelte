...
<script lang="typescript">
  ...
  export let usefacebook = true;
  export let usetwitter = true;
  export let usewhatsapp = true;
  export let uselink = true;
  export let useemail = true;
  ...
  $: twitterSharingUrl = `https://twitter.com/intent/tweet?text=${titleUE}&url=${urlUE}`;
  ...
  let showCopyToClipboardTooltip = false;
  ...
  function onClickCopyToClipboard(e: MouseEvent) {
    e.preventDefault();

    copyToClipboard(shareurl, e.currentTarget as HTMLElement);
  ...
  function copyToClipboard(str: string, element: HTMLElement) {
...
<style type="text/scss">
  ...
  .copy-to-clipboard-tip {
    ...
    &:before {
      ...
      border-width: 0.5em;
      border-style: solid;
      border-color: var(--tooltip-background-color, $fg) transparent transparent
        transparent;
    }
  }
</style>
  ...
  <a href="#share" class="opener" on:click={onClickToggle}>
    ...
    {#if !isOpen || !isOpenAnim}
      <svg
        ...
        viewBox="0 0 25 25">
        <path
          d="M12 2c5.514 0 10 4.486 10 10s-4.486 10-10 10-10-4.486-10-10
          ...
          4.425-5 4.464v-2.223c0 .001-3.78-.114-7 3.334z" />
      </svg>
    {:else}
      <svg
        ...
        <path
          ...
          4.552-4.547 4.592 1.405 1.405 4.555-4.596 4.591 4.55 1.403-1.416z" />
      </svg>
    {/if}

    <span class="title">{_.textShare(lang, textshare)}</span>

  </a>
  {#if isOpen}
    <ul class={`choices ${isOpenAnim ? 'shown' : 'hidden'}`}>
      {#if usefacebook}
        <li>
          <a href={facebookSharingUrl} target="_blank">
            <svg
              ...
              viewBox="0 0 35 35">
              <path
                d="M34.1,17.1C34.1,7.6,26.5,0,17.1,0S0,7.6,0,17.1c0,8.5,6.2,15.6,14.4,16.9V22h-4.3v-4.9h4.3v-3.8c0-4.3,2.5-6.6,6.4-6.6
          ...
          <a href={whatsAppSharingUrl} target="_blank">
            <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24">
              <path
                ...
                2.006-1.413.248-.695.248-1.29.173-1.414z" />
            </svg>
          </a>
        </li>
      {/if}
      ...
      {#if uselink}
        <li>
          <a
            ...
            on:click={onClickCopyToClipboard}
            target="_blank">
            ...
            <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24">
              <path
                d="M6.188 8.719c.439-.439.926-.801 1.444-1.087 2.887-1.591
                ...
                2.512-6.582.001-9.093-2.511-2.51-6.581-2.51-9.092 0z" />
            </svg>
          </a>
        </li>
      {/if}
      ...
      {#if useemail}
        <li>
          <a href={mailSharingUrl} target="_blank">
            <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24">
              <path
                d="M19 9.062s-5.188-.333-7 1.938c2-4.896 7-5.938 7-5.938v-2l5
